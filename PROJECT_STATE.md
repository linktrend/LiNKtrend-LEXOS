...
Last updated: 2026-05-12 (WP-04 + pre-merge security hardening on user_profiles and DELETE RLS)
...
| `supabase/migrations/` | Postgres migrations (WP-02 schema + WP-03 auth + **WP-04 RLS** + **WP-04 security hardening**). |
...
- **Migrations:** WP-02 (6) + WP-03 (1) + **WP-04 RLS (1)** + **WP-04 security hardening (1)** on live DB. WP-04 RLS applied via `apply_migration` MCP; hardening file `20260512100000_wp04_security_profile_lock_and_delete_policies.sql` applied via MCP **`execute_sql`** (three sequential statements; idempotent with migration file).
