...
test.describe("WP-12 strategy memo", () => {
  ...
  test("creates strategy memo and opens editor", async ({ page }) => {
    const creds = getE2ECredentials()!;
    await loginAsE2EUser(page, creds.email, creds.password);

    const matterId = await resolveMatterId(page);
    await page.goto(`/matters/${matterId}/strategy`);
    await expect(page.getByRole("heading", { name: "Strategy (W6)" })).toBeVisible({ timeout: 20_000 });

    await page.getByTestId("strategy-create-submit").click();

    const errorShown = await page
      .getByTestId("strategy-create-error")
      .waitFor({ state: "visible", timeout: 8000 })
      .then(() => true)
      .catch(() => false);
    if (errorShown) {
      const msg = await page.getByTestId("strategy-create-error").innerText();
      if (msg.includes("row-level security")) {
        test.skip(
          true,
          "Apply supabase/migrations/20260517100000_wp12_strategy_memos_rls.sql to the Supabase project in NEXT_PUBLIC_SUPABASE_URL (RLS blocks strategy_memos insert until then)."
        );
      }
    }

    await expect(page.getByTestId("strategy-memo-workspace")).toBeVisible({ timeout: 45_000 });
    await expect(page.getByTestId("strategy-memo-save")).toBeVisible();
  });
});
