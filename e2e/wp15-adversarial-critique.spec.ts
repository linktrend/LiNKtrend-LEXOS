...
test.describe("WP-15 adversarial critique", () => {
  ...
  test("creates adversarial critique and opens editor", async ({ page }) => {
    ...
    const argRls = await page
      .getByTestId("argument-create-error")
      .waitFor({ state: "visible", timeout: 8000 })
      .then(async () => page.getByTestId("argument-create-error").innerText())
      .catch(() => "");
    if (argRls.includes("row-level security")) {
      test.skip(
        true,
        "Argument draft RLS blocks insert — apply WP-14 argument_drafts RLS migration before WP-15 adversarial flow."
      );
    }
    ...
    const errorShown = await page
      ...
      .catch(() => false);
    if (errorShown) {
      const msg = await page.getByTestId("adversarial-create-error").innerText();
      if (msg.includes("row-level security")) {
        test.skip(
          true,
          "Apply supabase/migrations/20260520100000_wp15_adversarial_critiques_rls.sql to the Supabase project in NEXT_PUBLIC_SUPABASE_URL (RLS blocks adversarial_critiques insert until then)."
        );
      }
    }

    await expect(page.getByTestId("adversarial-critique-workspace")).toBeVisible({ timeout: 45_000 });
    await expect(page.getByTestId("adversarial-save")).toBeVisible();
  });
});
