...
test.describe("WP-13 research memo", () => {
  ...
  test("creates research memo and opens editor", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Research (W7)" })).toBeVisible({ timeout: 20_000 });

    await page.getByTestId("research-create-submit").click();

    const errorShown = await page
      .getByTestId("research-create-error")
      .waitFor({ state: "visible", timeout: 8000 })
      .then(() => true)
      .catch(() => false);
    if (errorShown) {
      const msg = await page.getByTestId("research-create-error").innerText();
      if (msg.includes("row-level security")) {
        test.skip(
          true,
          "Apply supabase/migrations/20260518100000_wp13_research_memos_rls.sql to the Supabase project in NEXT_PUBLIC_SUPABASE_URL (RLS blocks research_memos insert until then)."
        );
      }
    }

    await expect(page.getByTestId("research-memo-workspace")).toBeVisible({ timeout: 45_000 });
    await expect(page.getByTestId("research-memo-save")).toBeVisible();
  });
});
